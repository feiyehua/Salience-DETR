...
class NSAAttention(nn.Module):
    ...
    def _compressed_attention(self, 
        ...
        if key_padding_mask is not None:
            ...
            # 应用掩码
            scores = scores.masked_fill(compressed_mask > 0.5, -1e9)
    ...
    def _selection_attention(self, 
        ...
        if key_padding_mask is not None:
            ...
            scores = scores.masked_fill(key_padding_mask > 0.5, -1e9)
        ...
        # 计算所选键的注意力分数
        selected_scores = torch.einsum('bhqd,bhqkd->bhqk', q.unsqueeze(3), k_selected)
